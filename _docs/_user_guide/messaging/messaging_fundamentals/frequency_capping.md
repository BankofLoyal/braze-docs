...
Braze does not support a per second rate limit. Braze tries to evenly distribute the message sends throughout the minute, but cannot guarantee this. For example, if you have a campaign with a rate limit of 5,000 messages per minute, we will try to distribute the 5,000 requests evenly through the minute (about 84 messages per second), but there may be some variation in the per-second rate.
