...
- If there was a control group on the campaign, your user may have fallen into the control group.
  - You can check if this has happened by creating a segment with a received campaign variant filter, where the campaign variant is set to **Control**, and checking if your user fell into that segment. For email, SMS, and WhatsApp campaigns, data is also updated for all profiles that share the same channel identifier (email or phone number) when a delivery, open, or click occurs, so a user who shares an identifier with someone in a non-control variant could be attributed that variant and not appear in the control segment even if they were never directly sent the message. Cross-profile attribution does not apply the same way for in-app message campaigns.
