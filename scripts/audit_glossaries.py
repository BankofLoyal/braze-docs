...
def apply_fixes(report):
    ...
    Returns {"fixes_applied": N, "terms_added": M, "details": {...}}.
    """
    total_fixed = 0
    total_added = 0
    details = {}
    ...
    for lang_key, lang_data in sorted(report["languages"].items()):
        ...
        for term, term_mismatches in mismatches_by_term.items():
            ...
            if len(source_values) == 1:
                glossary[term] = term_mismatches[0]["source_value"]
                lang_fixed += 1
            else:
        ...
        details[lang_key] = {"fixed": lang_fixed, "added": lang_added}

    return {"fixes_applied": total_fixed, "terms_added": total_added,
            "details": details}
...
def run_audit(args):
    ...
    if getattr(args, "fix", False) and (total_mismatches > 0 or total_missing > 0):
        ...
        print(f"  Total: {fix_results['fixes_applied']} fixed, "
