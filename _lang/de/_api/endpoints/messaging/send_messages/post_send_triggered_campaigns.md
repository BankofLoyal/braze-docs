...
layout: api_page
page_type: reference
description: "In diesem Artikel finden Sie Einzelheiten über den Braze-Endpunkt „Kampagnen mit API-getriggerter Zustellung versenden"."

---
{% api %}
# Kampagnen-Nachrichten mit API-getriggerter Zustellung versenden
...
| --------- | ---------| --------- | ----------- |
|`campaign_id`|Erforderlich|String|Siehe [Kampagnen-Bezeichner]({{site.baseurl}}/api/identifier_types/). |
...
|`recipients`| Optional | Array | Siehe [Empfänger:innen-Objekt]({{site.baseurl}}/api/objects_filters/recipient_object/).<br><br>Wenn `send_to_existing_only` `false` ist, muss ein Attribut-Objekt enthalten sein.<br><br>Wenn `recipients` nicht angegeben und `broadcast` auf true gesetzt ist, wird die Nachricht an das gesamte Segment gesendet, das im Braze-Dashboard als Zielgruppe der Kampagne konfiguriert ist. <br><br> Wenn `email` der Bezeichner ist, müssen Sie [`prioritization`]({{site.baseurl}}/api/endpoints/user_data/post_user_identify#identifying-users-by-email) in das Empfänger:innen-Objekt aufnehmen. |
...
- Das Empfänger:innen-Array kann bis zu 50 Objekte enthalten, wobei jedes Objekt einen einzelnen Nutzer-Bezeichner (`external_user_id`, `user_alias` oder `email`) und ein optionales `trigger_properties`-Objekt enthält.
- Wenn `send_to_existing_only` `true` ist (Standard), sendet Braze die Nachricht nur an bestehende Nutzer:innen. Wenn der Bezeichner im Empfänger:innen-Objekt (`external_user_id`, `user_alias` oder `email`) keiner vorhandenen Nutzer:in zugeordnet werden kann, gibt die API dennoch eine `201`-Erfolgsantwort mit einer `dispatch_id` zurück, jedoch wird der Versand intern abgebrochen und keine Nachricht zugestellt. Für `external_user_id` wird das Ergebnis als `Unknown External User ID` protokolliert und es wird kein Currents-Ereignis generiert. Wenn der Wert auf `false` gesetzt und ein Attribut-Objekt bereitgestellt wird, erstellt Braze eine neue Nutzer:in, falls noch keine vorhanden ist. Bitte beachten Sie, dass die Einstellung `send_to_existing_only` auf `false` für Nutzer-Aliase nicht unterstützt wird – neue Nutzer:innen, die nur über einen Alias verfügen, können über diesen Endpunkt nicht erstellt werden. Um an eine Nutzer:in zu senden, die nur über einen Alias verfügt, muss diese bereits in Braze vorhanden sein.

{% alert important %}
Eine `201`-Antwort mit einer `dispatch_id` bestätigt, dass Braze die Anfrage akzeptiert hat, garantiert jedoch nicht die Zustellung der Nachricht. Die Zustellung kann fehlschlagen, wenn die Nutzer:in nicht gefunden werden kann, abgemeldet ist oder keine gültige Kontaktadresse für den Kanal hat.
{% endalert %}

Der Abo-Gruppenstatus einer Nutzer:in kann über den Parameter `subscription_groups` innerhalb des `attributes`-Objekts aktualisiert werden. Weitere Einzelheiten finden Sie unter [Nutzer:innen-Attribute-Objekt]({{site.baseurl}}/api/objects_filters/user_attributes_object).

{% alert note %}
Der Parameter `segment_id` wird für diesen Endpunkt nicht unterstützt. Um ein Segment anzusprechen, konfigurieren Sie das Segment in den Zielgruppen-Einstellungen der Kampagne im Braze-Dashboard und verwenden Sie `"broadcast": true`, oder nutzen Sie den `audience`-Parameter mit [Connected Audience]({{site.baseurl}}/api/objects_filters/connected_audience/)-Filtern.
{% endalert %}
