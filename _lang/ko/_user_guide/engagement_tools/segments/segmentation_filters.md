...
glossaries:
  ...
  - name: Received Message from Campaign or Canvas with Tag
    description: "특정 캠페인 또는 특정 태그가 포함된 캔버스를 수신했는지 여부에 따라 사용자를 세분화합니다. \"캠페인에서 받은 메시지\" 및 \"캔버스 단계에서 받은 메시지\"와 달리, 이 필터는 중복 메시지를 받은 동일한 이메일 또는 전화번호를 가진 모든 사용자를 캡처합니다.<br><br> 콘텐츠 카드, 배너(캠페인만 해당) 및 인앱 메시지의 경우, 카드 또는 인앱 메시지가 전송되는 시점이 아니라 사용자가 노출을 기록하는 시점입니다.<br><br>푸시 및 웹훅의 경우 사용자에게 메시지가 전송되는 시점입니다.<br><br> WhatsApp의 경우, 메시지가 사용자의 기기로 전달되는 시점이 아니라 마지막 메시지 API 요청이 WhatsApp으로 전송되는 시점입니다. <br><br>이메일의 경우, 이메일 요청이 이메일 서비스 제공업체에 전송되는 시점입니다(실제로 전달되는지 여부와 관계없이). 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 최초 전송 시에는 특정 타겟 사용자의 프로필만 업데이트됩니다. <br>- 이메일이 전달되거나 사용자가 이메일 또는 이메일의 링크를 열면, 해당 이메일 주소를 공유하는 모든 사용자가 메시지를 받은 것으로 나타납니다.<br><br>SMS 및 RCS의 경우 마지막 메시지가 SMS 또는 RCS 제공업체에 전달된 시점입니다. 메시지가 사용자의 기기에 전달되었다는 보장은 없습니다."
    tags:
      - Retargeting
  ...
  - name: Last Received SMS
    description: "마지막 SMS, MMS 또는 RCS 메시지가 SMS 또는 RCS 제공업체에 전달된 시간을 기준으로 사용자를 세분화합니다. 메시지가 사용자의 기기에 전달되었다는 보장은 없습니다. (24시간 기간)<br><br>시간대:<br>회사 표준 시간대"
    tags:
      - Retargeting
  ...
  - name: Clicked/Opened Campaign
    description: "특정 캠페인과의 상호작용을 기준으로 필터링합니다. 이메일 메시징의 경우 열기 이벤트에는 기계적 열기와 비기계적 열기가 모두 포함됩니다.<br><br> 이메일의 경우 '모든 이메일 열기(기계적 열기)' 및 '모든 이메일 열기(기타 열기)'로 필터링하는 옵션도 포함되어 있습니다. 탈퇴 링크 및 환경설정 센터 클릭은 이 필터에 포함되지 않습니다. 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 이메일을 열거나 클릭하면 같은 이메일 주소를 가진 다른 모든 사용자도 프로필이 업데이트됩니다. <br>- 메시지가 전송된 후 열기 또는 클릭 전에 원래 사용자가 이메일 주소를 변경하면 원래 사용자 대신 해당 이메일 주소를 가진 나머지 모든 사용자에게 열기 또는 클릭이 적용됩니다.<br><br>SMS 및 RCS의 경우 상호작용은 다음과 같이 정의됩니다:<br>- 사용자가 마지막으로 특정 키워드 카테고리와 일치하는 답장 SMS 또는 RCS를 보냈습니다. 이는 이 전화번호를 사용하는 모든 사용자가 가장 최근에 받은 캠페인에 기여됩니다. 캠페인은 지난 4시간 이내에 수신된 것이어야 합니다.<br>- 사용자가 특정 캠페인에서 사용자 클릭 추적이 켜져 있는 SMS 또는 RCS 메시지의 단축 링크를 마지막으로 선택했습니다."
    tags:
      - Retargeting
  - name: Clicked/Opened Campaign or Canvas With Tag
    description: "특정 태그가 있는 특정 캠페인과의 상호작용을 기준으로 필터링합니다. 이메일 메시징의 경우 열기 이벤트에는 기계적 열기와 비기계적 열기가 모두 포함됩니다.<br><br> 이메일의 경우 '모든 이메일 열기(기계적 열기)' 및 '모든 이메일 열기(기타 열기)'를 기준으로 필터링하는 옵션이 포함됩니다. 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 이메일을 열거나 클릭하면 같은 이메일 주소를 가진 다른 모든 사용자도 프로필이 업데이트됩니다. <br>- 메시지가 전송된 후 열기 또는 클릭 전에 원래 사용자가 이메일 주소를 변경하면 원래 사용자 대신 해당 이메일 주소를 가진 나머지 모든 사용자에게 열기 또는 클릭이 적용됩니다.<br><br>SMS 및 RCS의 경우 상호작용은 다음과 같이 정의됩니다:<br>- 사용자가 마지막으로 특정 키워드 카테고리와 일치하는 답장 SMS 또는 RCS를 보냈습니다. 이는 이 전화번호를 사용하는 모든 사용자가 가장 최근에 받은 캠페인에 기여됩니다. 캠페인은 지난 4시간 이내에 수신된 것이어야 합니다.<br>- 사용자가 태그가 있는 특정 캠페인 또는 캔버스 단계에서 사용자 클릭 추적이 켜져 있는 SMS 또는 RCS 메시지의 단축 링크를 마지막으로 선택한 경우."
    tags:
      - Retargeting
  - name: Clicked/Opened Step
    description: "특정 캔버스 구성요소와의 상호작용을 기준으로 필터링합니다. 이메일 메시징의 경우 열기 이벤트에는 기계적 열기와 비기계적 열기가 모두 포함됩니다.<br><br>이메일의 경우 '모든 이메일 열기(기계적 열기)' 및 '모든 이메일 열기(기타 열기)'를 기준으로 필터링하는 옵션이 포함됩니다.<br><br>SMS 및 RCS의 경우 상호작용은 다음과 같이 정의됩니다:<br>- 사용자가 마지막으로 특정 키워드 카테고리와 일치하는 답장 SMS 또는 RCS를 보냈습니다. 이는 이 전화번호를 사용하는 모든 사용자가 가장 최근에 받은 캠페인에 기여됩니다. 캠페인은 지난 4시간 이내에 수신된 것이어야 합니다. <br>- 사용자가 지정된 캔버스 단계에서 사용자 클릭 추적이 켜져 있는 SMS 또는 RCS 메시지의 단축 링크를 마지막으로 선택했습니다."
    tags:
      - Retargeting
  ...
  - name: Last Sent Specific SMS Inbound Keyword Category
    description: 특정 키워드 카테고리 내의 특정 구독 그룹에 마지막으로 SMS, MMS 또는 RCS를 보낸 시점을 기준으로 사용자를 세분화합니다.
    tags:
      - Retargeting
  ...
  - name: Last Received Any Message
    description: "마지막으로 수신한 메시지를 확인하여 사용자를 세분화합니다. (24시간 기간)<br><br> 콘텐츠 카드, 배너 및 인앱 메시지의 경우, 카드 또는 인앱 메시지가 마지막으로 전송된 시점이 아니라 사용자가 마지막으로 노출을 기록한 시점입니다.<br><br>푸시 및 웹훅의 경우 사용자에게 메시지가 전송된 시점입니다.<br><br> WhatsApp의 경우, 메시지가 사용자의 기기로 전달된 시점이 아니라 마지막 메시지 API 요청이 WhatsApp으로 전송된 시점입니다. <br><br>이메일의 경우, 이메일 요청이 이메일 서비스 제공업체에 전송되는 시점입니다(실제로 전달되는지 여부와 관계없이). 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 최초 전송 시에는 특정 타겟 사용자의 프로필만 업데이트됩니다. <br>- 이메일이 전달되거나 사용자가 이메일 또는 이메일의 링크를 열면, 해당 이메일 주소를 공유하는 모든 사용자가 메시지를 받은 것으로 나타납니다.<br><br>SMS 및 RCS의 경우 마지막 메시지가 SMS 또는 RCS 제공업체에 전달된 시점입니다. 메시지가 사용자의 기기에 전달되었다는 보장은 없습니다.<br><br>예시:<br>마지막으로 받은 메시지 1일 전 미만 = 24시간 전 미만<br><br>시간대:<br>회사 표준 시간대"
    tags:
      - Retargeting
  - name: Last Engaged With Message
    description: "사용자가 메시징 채널(배너, 콘텐츠 카드, 이메일, 인앱, SMS, RCS, 푸시, WhatsApp) 중 하나를 마지막으로 클릭하거나 연 시간을 기준으로 사용자를 세분화합니다. 이메일 메시징의 경우 열기 이벤트에는 기계적 열기와 비기계적 열기가 모두 포함됩니다. (24시간 기간)<br><br>이메일의 경우, 이메일 요청이 이메일 서비스 제공업체에 전송되는 시점입니다(실제로 전달되는지 여부와 관계없이). 여기에는 '모든 이메일 열기(기계적 열기)' 및 '모든 이메일 열기(기타 열기)'로 필터링하는 옵션도 포함되어 있습니다. 여러 사용자가 동일한 이메일 주소를 공유하는 경우:<br>- 최초 전송 시에는 특정 타겟 사용자의 프로필만 업데이트됩니다. <br>- 이메일이 전달되거나 사용자가 이메일 또는 이메일의 링크를 열면, 해당 이메일 주소를 공유하는 모든 사용자가 메시지를 받은 것으로 나타납니다.<br><br>SMS 및 RCS의 경우, 사용자 클릭 추적이 켜져 있는 메시지에서 사용자가 마지막으로 단축 링크를 선택한 시점을 의미합니다.<br><br>시간대:<br>회사 표준 시간대"
    tags:
      - Retargeting
  - name: Clicked card
  ...
  - name: Subscription Group
    description: 이메일, SMS, MMS, RCS 또는 WhatsApp의 구독 그룹별로 사용자를 세분화합니다. 아카이브된 그룹은 표시되지 않으며 사용할 수 없습니다.
    tags:
  ...
  - name: Segment Cohorts
    description: Segment를 사용하는 클라이언트는 Segment에서 코호트를 선택하고 가져와서 세그먼트를 보완할 수 있습니다.
    tags:
      - Cohort membership
  ...
  - name: Number of Twitter Followers
    ...
    tags:
      - Social activity
  - name: Phone Number
    description: "E.164 형식의 전화번호 필드를 기준으로 사용자를 세분화합니다.<br><br> 전화번호가 Braze에 전송되면, Braze는 SMS, RCS 및 WhatsApp 채널을 통해 전송하는 데 사용되는 <a href=\"/docs/user_guide/message_building_by_channel/sms/phone_numbers/user_phone_numbers/#importing-phone-numbers\">e.164 형식</a>으로 강제 변환하려고 시도합니다. 전화번호의 형식이 올바르지 않으면 강제 변환 프로세스가 실패하여 사용자 프로필에 형식화되지 않은 전화번호가 있지만 발신 전화번호는 없는 결과가 발생할 수 있습니다. 이 세그먼트 필터는 e.164 형식의 전화번호(사용 가능한 경우)로 사용자를 반환합니다.<br><br>활용 사례:<br> - 이 필터를 사용하면 SMS, RCS 또는 WhatsApp 메시지를 보낼 때 가장 정확한 타겟 오디언스 규모를 파악할 수 있습니다.  <br>- 이 필터와 함께 정규표현식(정규식)을 사용하여 특정 국가 코드가 있는 전화번호로 세분화합니다. <br>- 이 필터를 사용하여 e.164 강제 변환 프로세스에 실패한 전화번호로 사용자를 세분화합니다."
    tags:
      - Other Filters
---